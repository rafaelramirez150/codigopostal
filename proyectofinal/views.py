"""
 El código consiste en vistas y funciones Django para manejar la búsqueda de códigos postales basados en
 diferentes criterios y devolver los resultados en formato JSON.
 
:param request: El parámetro `request` en Django representa la petición HTTP que se envió a la
 vista. Contiene información sobre la petición, como el método de petición (GET, POST, etc.),
 cualquier dato enviado en la petición (POST, etc.), etc.
 datos enviados en la petición (datos POST, parámetros de consulta), información de la sesión del usuario, etc.
 
 La vista utiliza :return: El código proporcionado contiene varias vistas y funciones para gestionar
 diferentes funciones de búsqueda relacionadas con códigos postales y localidades
 relacionadas con códigos postales y ubicaciones.
 """
...
def index(request):
    ...
    Si se realiza una solicitud POST, obtiene el estado, municipio y asentamiento y ejecuta el
    stored procedure 'buscar_por_estado_municipio_asentamiento' para buscar por estado, municipio y asentamiento.
    """
...
def execute_stored_procedure(procedure_name, *args):
    """
    Función para ejecutar stored procedure en la base de datos.
    
    Esta función recibe el nombre del stored procedure y los argumentos necesarios para ejecutarlo.
    Utiliza la conexión a la base de datos y el cursor para llamar al stored procedure
...
def buscar_por_codigo_postal(request):
    ...
    Si se realiza una solicitud POST, obtiene el código postal y ejecuta el stored procedure
...
def buscar_por_asentamiento(request):
    ...
    Si se realiza una solicitud POST, obtiene el nombre del asentamiento y ejecuta el stored procedure
...
def buscar_por_municipio(request):
    ...
    Si se realiza una solicitud POST, obtiene el nombre del municipio y ejecuta el stored procedure
...
def buscar_por_estado(request):
    ...
    Si se realiza una solicitud POST, obtiene el nombre del estado y ejecuta el stored procedure
...
def buscar_por_estado_municipio_asentamiento(request):
    ...
    Esta vista renderiza la plantilla 'index.html' y muestra los resultados de la búsqueda por estado, municipio
    y asentamiento.
    Si se realiza una solicitud POST, obtiene el estado y municipio, filtra los asentamientos por estado
    y municipio, y devuelve la lista de asentamientos como una respuesta JSON.
    """
    if request.method == 'POST':
        estado = request.POST.get('nombre_estado')
        municipio = request.POST.get('nombre_municipio')
